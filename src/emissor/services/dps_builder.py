...
def build_dps(
    ...
    _sub(cserv, "cTribNac", invoice.c_trib_nac or emitter.c_trib_nac)
    _sub(cserv, "xDescServ", invoice.x_desc_serv or emitter.x_desc_serv)
    _sub(cserv, "cNBS", invoice.c_nbs or emitter.c_nbs)
    com_ext = _sub(serv, "comExt")
    _sub(com_ext, "mdPrestacao", invoice.md_prestacao or "4")
    _sub(com_ext, "vincPrest", invoice.vinc_prest or "0")
    _sub(com_ext, "tpMoeda", invoice.tp_moeda or emitter.tp_moeda)
    _sub(com_ext, "vServMoeda", invoice.valor_usd)
    _sub(com_ext, "mecAFComexP", invoice.mec_af_comex_p or client.mec_af_comex_p)
    _sub(com_ext, "mecAFComexT", invoice.mec_af_comex_t or client.mec_af_comex_t)
    _sub(com_ext, "movTempBens", invoice.mov_temp_bens or "1")
    _sub(com_ext, "mdic", invoice.mdic or "0")
    ...
    _sub(trib_mun, "tribISSQN", invoice.trib_issqn or "3")
    _sub(trib_mun, "cPaisResult", invoice.c_pais_result or emitter.c_pais_result)
    _sub(trib_mun, "tpRetISSQN", invoice.tp_ret_issqn or "1")
    trib_fed = _sub(trib, "tribFed")
    piscofins = _sub(trib_fed, "piscofins")
    _sub(piscofins, "CST", invoice.cst_pis_cofins or "08")
    tot_trib = _sub(trib, "totTrib")
    p_tot = _sub(tot_trib, "pTotTrib")
    _sub(p_tot, "pTotTribFed", invoice.p_tot_trib_fed or "0.00")
    _sub(p_tot, "pTotTribEst", invoice.p_tot_trib_est or "0.00")
    _sub(p_tot, "pTotTribMun", invoice.p_tot_trib_mun or "0.00")
